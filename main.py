...
shape_I = Tetronimo([[0, 3], [0, 4], [0, 5], [0, 6]],
                    blue_square, [1, 1, 1, 1])
shape_J = Tetronimo([[0, 3], [0, 4], [0, 5], [-1, 3]],
                    brown_square, [1, 1, 2, 2])
shape_L = Tetronimo([[0, 3], [0, 4], [0, 5], [-1, 5]],
                    orange_square, [1, 2, 2, 1])
shape_O = Tetronimo([[0, 4], [0, 5], [-1, 4], [-1, 5]],
                    yellow_square, [1, 1, 1, 1])
shape_S = Tetronimo([[0, 3], [0, 4], [-1, 4], [-1, 5]],
                    green_square, [2, 2, 2, 2])
shape_T = Tetronimo([[0, 3], [0, 4], [0, 5], [-1, 4]],
                    purple_square, [1, 1, 3, 0])
shape_Z = Tetronimo([[0, 4], [0, 5], [-1, 3], [-1, 4]],
                    red_square, [0, 1, 0, 2])
...
def get_random_shape():
    ...
    random_shape = [random_shape.starting_pos[:],
                    random_shape.colour, random_shape.rotation_points]
    global is_new_shape
    is_new_shape = True
        ...
        for square in shape:
            ...
            if (0 <= new_square_col < num_of_cols) and (0 <= new_square_row < num_of_rows):
                ...
                if (square_checking != empty_square) and ([new_square_row, new_square_col] not in old_shape_pos):
                    ...
                    new_shape_pos.append(
                        [new_square_row, new_square_col])  # store pos
...
def rotate_shape(shape, direction, rotation_point_index, shape_colour):
    ...
    for square in shape:
        ...
        if direction == 'clockwise':
            new_square_row = (
                square_col - rotation_point[1]) + rotation_point[0] + rot_adjustments.get(shape_colour)[rotation_pos-1][0]
            print('Adjustment made: ' +
                  str(rot_adjustments.get(shape_colour)[rotation_pos-1][0]))
            new_square_col = - \
                (square_row - rotation_point[0]) + rotation_point[1] + \
            ...
            new_square_row = - \
                (square_col - rotation_point[1]) + rotation_point[0]
            new_square_col = (
                square_row - rotation_point[0]) + rotation_point[1]
        new_shape.append([new_square_row, new_square_col]
                         )  # store pos of rotated square
        if (0 <= square_col < num_of_cols) and (0 <= square_row < num_of_rows):  # if on board
            # make empty old square pos
            board[square_row][square_col] = empty_square

    new_shape = do_wall_kicks(
        new_shape, shape, shape_colour, 0)  # offset shape

    # sort so that bottom squares are first in list
    ...
    # if not same as old unrotated shape (in case places at start pos)
    if new_shape != shape:
    ...
    for i in range(amnt_to_check):
        ...
        for square in cur_shape_pos:
            ...
            if (0 <= square_col < num_of_cols):  # if current column spot will fit
                # if spot with column position changed won't fit
                ...
                if (0 <= square_row + movement_amnt < num_of_rows):  # if new square row pos is on board
                    # get the square to check if empty
                    square_checking = board[square_row +
                                            movement_amnt][square_col + h_movement]
                    # if square is not empty / won't be when other parts of shape have moved
                    if (square_checking != empty_square) and ([square_row + movement_amnt, square_col + h_movement] not in cur_shape_pos):
                        ...
                        if (square_checking != empty_square) and ([square_row + movement_amnt, square_col + h_movement] not in cur_shape_pos):
                            if movement_amnt == 1:
                                ...
                                print('Square checking: ' + str(square_row +
                                      movement_amnt) + ', ' + str(square_col + h_movement))
...
async def run_game(msg, cur_shape):
    ...
    if rotate_clockwise == True and cur_shape_colour != yellow_square:
        cur_shape_pos = rotate_shape(
            cur_shape_pos, 'clockwise', cur_shape[2][rotation_pos], cur_shape_colour)  # rotate shape
        cur_shape = [cur_shape_pos, cur_shape_colour,
                     cur_shape[2]]  # update shape
    ...
    if next_space_free:
        for square in cur_shape_pos:
            ...
            square_col = square[1]
            if (0 <= square_row + movement_amnt < num_of_rows):  # if new square row pos is on board
                # get square to change
                square_changing = board[square_row +
                                        movement_amnt][square_col + h_movement]
                # changes square colour to colour of shape
                board[square_row + movement_amnt][square_col +
                                                  h_movement] = cur_shape_colour
                if is_new_shape == True:
                    is_new_shape = False  # has been placed, so not new anymore
                # stops from wrapping around list and changing colour of bottom rows.
                if square_row > -1:
                    # make old square empty again
                    board[square_row][square_col] = empty_square
                cur_shape_pos[square_num_in_shape] = [
                    square_row + movement_amnt, square_col + h_movement]  # store new pos of shape square
    ...
    if not game_over:
        # Update board
        embed = discord.Embed(
            description=format_board_as_str(), color=embed_colour)
        ...
        desc = 'Score: {} \n Lines: {} \n \n Press ▶ to play again.'.format(
            points, lines)
        embed = discord.Embed(
            title='GAME OVER', description=desc, color=embed_colour)
        await msg.edit(embed=embed)
